...
CONSUME_MATH_TOKEN_NAME = "consume_ablation_prompt_MATH_token.json"
RUN_MATH_LOG_NAME = "run_ablation_prompt_MATH.log"
MATH_ACC_NAME = "ablation_prompt_MATH_acc.json"
MATH_CSV_NAME = "ablation_prompt_math_dataset_summary.csv"
...
def main() -> None:
    ...
    if not os.path.exists(filepath_config["Ablation_results_prompt_MATH_dir"]):
        os.makedirs(filepath_config["Ablation_results_prompt_MATH_dir"])
    ...
    logger = setup_logger(run_MATH_log_path, "ablation_prompt_MATH_log")
    ...
    math_acc_path = os.path.join(filepath_config["Ablation_results_prompt_MATH_dir"], MATH_ACC_NAME)
    ...
    for subject, df in type_df:
        ...
        subject_result_path = os.path.join(filepath_config["Ablation_results_prompt_MATH_dir"], f"{subject}_result.json")
        ...
        for j in range(df.shape[0]):
            ...
            ans = main_agent.forword(query, max_tree_depth=3, attempts=3, use_prompt_opt_flag=False)
